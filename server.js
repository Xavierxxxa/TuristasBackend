...
function isAuthenticatedRegister({email}){
...
server.post('/auth/login', (req, res) => {
  ...
  const access_token = createToken({email, password})
  console.log("Access Token:" + access_token);
  const dadoslogin = userdb.users.find(users => users.email === email)
  console.log("Nome Logado: " + dadoslogin)
  res.status(200).json({access_token, dadoslogin})
})
